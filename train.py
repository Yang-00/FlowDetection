...
if(not os.path.isdir("train")):
        z = zipfile.ZipFile("G:/WHU/term4/shixun/dataset/train_new.zip", 'r')
        z.extractall("")
        z.close()

start = time.time()
#把图片对应的标签装入字典
f = open('G:/WHU/term4/shixun/dataset/train.json')
content = json.load(f)
...
print(content['annotations'][1]['name'])
#读取解压文件里的信息
zfile = zipfile.ZipFile("G:/WHU/term4/shixun/dataset/train_new.zip")
l = []
...
name = 'G:/WHU/term4/shixun/dataset/train_new/8538edb45aaf7df78336aa5b49001be6.jpg'
imm = Image.open(name)
print(imm.size)

plt.imshow(imm)
im_size = (int(imm.size[0]/2),int(imm.size[1]/2))
aaa = imm.resize(im_size)
print(aaa.size)
#图片操作
...
for a in range(len(qt)):
    for x in range(img.size[0]):
        for y in range(img.size[1]):
            if(x > (qt[a][0]-10) and x < (qt[a][0]+10) and y > qt[a][1]-10 and y < (qt[a][1]+10)):  #取坐标范围正负10的像素
                img.putpixel((x,y),(255,0,0))           #对所取范围的像素变成红色
plt.imshow(img)
k = ground(im,qt)
...
def train_set():
    def inner():
        for ig_index in range(2000):  # 遍历所有图片
            ...
            if content['annotations'][ig_index]['name'] == 'G:/WHU/term4/shixun/dataset/train_new/8538edb45aaf7df78336aa5b49001be6.jpg': continue
            if content['annotations'][ig_index]['name'] == 'G:/WHU/term4/shixun/dataset/train_new/377df0a7a9abc44e840e938521df3b54.jpg': continue
...
data2 = next(iter)
data1 = next(iter)
data = next(iter)  # 生成的第一个数据
print(data[0].shape, data[1].shape)  # 数据格式
da = data[0].transpose()

plt.imshow(da)

dat = data[1] + 0.5
plt.imshow(data[1], cmap=CM.jet)
print("Sum = ", np.sum(data[1]))
BATCH_SIZE= 20    #每次取10张
# 设置训练reader
train_reader = paddle.batch(
    paddle.reader.shuffle(
        train_set(), buf_size=50),
...
if (not os.path.isdir("test")):
    z = zipfile.ZipFile("G:/WHU/term4/shixun/dataset/test_new.zip", 'r')
    z.extractall("")
    z.close()
# 测试图片
import numpy as np
...
test_zfile = zipfile.ZipFile("G:/WHU/term4/shixun/dataset/test_new.zip")
l_test = []
